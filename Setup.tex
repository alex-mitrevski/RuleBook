...
\begin{itemize}
	...
	\item \textbf{Tables:} The \abb{LOC} will set up and randomly assign team tables.
	\item \textbf{Arena:} The \Arenas{} are available to all teams of the respective league. The \abb{OC} may schedule special test or mapping slots in which arena access is limited. Note, however, that furnishing may not be complete yet.
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\begin{itemize}
			\item Allowed \abb{DSPL} modifications (see \ref{sec:rules:robotappearance_spl})
	...
	\end{itemize}
	\item \textbf{Re-Inspection:} If the robot is not approved, it is the responsibility of the team to get the approval later. This can be done by retrying directly after the regular \RobotInspection{} schedule or by asking the \abb{TC} to be inspected at a later time.
\end{itemize}
