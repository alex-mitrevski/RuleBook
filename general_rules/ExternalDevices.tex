...
During a \Testblock{} (see~\ref{sec:rules:schedule}), only two persons are allowed in the \abb{ECRA} at any time: one team member of each of the two teams up next. No peripherals (e.g., screens, mice, keyboards) are allowed to be present. Laptops can only be placed if the team is up next, and must be removed as soon as the test finishes.
